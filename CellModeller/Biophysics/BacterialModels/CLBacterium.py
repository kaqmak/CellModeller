...
class CLBacterium:
    ...
    def init_kernels(self):
        ...
        self.vadd = ElementwiseKernel(
            self.context,
            "float8 *res, const float8 *in1, const float8 *in2",
            "res[i] = in1[i] + in2[i]",
            "vecadd",
        )
        ...
        self.vmulk = ElementwiseKernel(
            self.context,
            "float8 *res, const float k, const float8 *in1",
            "res[i] = k*in1[i]",
            "vecmulk",
        )
        self.vnorm = ElementwiseKernel(
            self.context, "float8 *res, const float8 *in1", "res[i] = dot(in1[i], in1[i]", "vecnorm"
        )

        # cell geometry kernels
        self.calc_cell_area = ElementwiseKernel(
            self.context,
            "float* res, float* r, float* l",
            "res[i] = 2.f*3.1415927f*r[i]*(2.f*r[i]+l[i])",
            "cell_area_kern",
        )
        self.calc_cell_vol = ElementwiseKernel(
            self.context,
            "float* res, float* r, float* l",
            "res[i] = 3.1415927f*r[i]*r[i]*(2.f*r[i]+l[i])",
            "cell_vol_kern",
        )
    ...
    def load_2_cells(self):
        ...
        self.cell_dirs.put(
            [0, 1], [(root2 / 2.0, root2 / 2.0, 0, 0), (-root2 / 2.0, root2 / 2.0, 0, 0)]
        )
    ...
    def progress_finalise(self):
        ...
        if self.frame_no % 10 == 0:
            print(
                "% 8i    % 8i cells    % 8i contacts    %f hour(s) or %f minute(s) or %f second(s)"
                % (
                    self.frame_no,
                    self.n_cells,
                    self.n_cts,
        ...
        if self.computeNeighbours:  # populate cellstate.neighbours
            for n in range(self.cell_cts[i]):
                if self.neighbours[i, n] not in state.neighbours:
                    state.neighbours.append(
                        self.neighbours[i, n]
                    )  # ids of all cells in physical contact
    ...
    def sort_cells(self):
        ...
        """
        self.sorted_ids.put(
            numpy.arange(self.n_cells), numpy.argsort(self.cell_sqs[: self.n_cells])
        )
    ...
    def find_contacts(self, predict=True):
        ...
        self.program.find_sphere_contacts(
            self.queue,
            (self.n_cells,),
            None,
            numpy.int32(self.max_cells),
    ...
    def CGSSolve(self, dt, alpha, substep=False):
        ...
        self.vsub(
            self.res_dev[0 : self.n_cells],
            self.rhs_dev[0 : self.n_cells],
            self.BTBx_dev[0 : self.n_cells],
        )

        # p = res
        cl.enqueue_copy(
            self.queue, self.p_dev[0 : self.n_cells].data, self.res_dev[0 : self.n_cells].data
        )
        ...
        if math.sqrt(rsold / self.n_cells) < self.cgs_tol:
            if self.printing and self.frame_no % 10 == 0:
                print(
                    "% 5i" % self.frame_no
                    + "% 6i cells  % 6i cts  % 6i iterations  residual = %f"
                    % (self.n_cells, self.n_cts, 0, math.sqrt(rsold / self.n_cells))
                )
            return (0.0, rsold)
        ...
        for iter in range(max_iters):
            # Ap
            self.calculate_Ax(
                self.Ap_dev[0 : self.n_cells], self.p_dev[0 : self.n_cells], dt, alpha
            )
            ...
            self.vaddkx(
                self.deltap_dev[0 : self.n_cells],
                alpha,
                self.deltap_dev[0 : self.n_cells],
        ...
        if self.printing and self.frame_no % 10 == 0:
            print(
                "% 5i" % self.frame_no
                + "% 6i cells  % 6i cts  % 6i iterations  residual = %f"
                % (self.n_cells, self.n_cts, iter + 1, math.sqrt(rsnew / self.n_cells))
            )
    ...
    def calc_cell_geom(self):
        ...
        # update geometry
        self.calc_cell_area(
            self.cell_areas_dev[0 : self.n_cells],
            self.cell_rads_dev[0 : self.n_cells],
            self.cell_lens_dev[0 : self.n_cells],
        )
        self.calc_cell_vol(
            self.cell_vols_dev[0 : self.n_cells],
            self.cell_rads_dev[0 : self.n_cells],
            self.cell_lens_dev[0 : self.n_cells],
        )
