...
class PyGLCMViewer(PyGLWidget):
    ...
    def getOpenCLPlatform(self):
        ...
        platdict = dict(list(zip(platlist, list(range(len(platlist))), strict=False)))
        ...
        qsPlatformName, ok = QInputDialog.getItem(
            self, "Choose OpenCL platform", "Available platforms:", platlist, editable=False
        )
        if not ok:
    ...
    def getOpenCLDevice(self):
        ...
        devdict = dict(list(zip(devlist, list(range(len(devlist))), strict=False)))
