...
class Simulator:
    ...
    def init_data_output(self):
        ...
        outputFileRoot = (
            ...
            else self.moduleName + "-" + time.strftime("%y-%m-%d-%H-%M", startTime)
        )
    ...
    def init_cl(self, platnum, devnum):
        ...
        if len(devices) <= devnum:
            print(
                "Specified OpenCL device number (%d) does not exist on platform %s."
                % (devnum, platform)
            )
            print("Options are:")
        ...
        self.CLContext = cl.Context(
            properties=[(cl.context_properties.PLATFORM, platform)], devices=[device]
        )
