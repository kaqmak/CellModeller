...
class CLEulerSigIntegrator:
    ...
    def makeViews(self):
        ...
        self.specLevel = self.levels[
            self.signalDataLen : self.signalDataLen + self.maxSpecDataLen
        ].reshape(self.maxCells, self.nSpecies)
        # Rate views (references) to the data
        self.signalRate = self.rates[0 : self.signalDataLen]
        self.specRate = self.rates[
            self.signalDataLen : self.signalDataLen + self.maxSpecDataLen
        ].reshape(self.maxCells, self.nSpecies)
    ...
    def initArrays(self):
        ...
        self.cellSigLevels_dev = cl_array.zeros(
            self.queue, (self.maxCells, self.nSignals), dtype=numpy.float32
        )
        self.signalLevel_dev = cl_array.zeros(self.queue, self.gridDim, dtype=numpy.float32)
        self.specLevel_dev = cl_array.zeros(
            self.queue, (self.maxCells, self.nSpecies), dtype=numpy.float32
        )
        self.specRate_dev = cl_array.zeros(
            self.queue, (self.maxCells, self.nSpecies), dtype=numpy.float32
        )
    ...
    def dydt(self):
        ...
        cellSigRates = self.cellSigRates[0 : self.nCells, :, :].reshape(
            (self.nCells * 8, self.nSignals)
        )
        cellSigRates = cellSigRates[order]
        cellSigRates.cumsum(axis=0, out=cellSigRates)
        (u, index) = unique_stable(
            gridIdxs[::-1], True
        )  # first occurance of each key in reverse keys
        index = len(gridIdxs) - 1 - index  # last occurance in forward keys
        cellSigRates = cellSigRates[index]
        idxs = gridIdxs[index]
        cellSigRates[1:] = (
            cellSigRates[1:] - cellSigRates[:-1]
        )  # difference in cumsums is sum for each index

        signalRate = self.signalRate.reshape(
            (self.nSignals, self.gridDim[1] * self.gridDim[2] * self.gridDim[3])
        )
