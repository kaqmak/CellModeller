...
class GridDiffusion:
    ...
    def __init__(
        self, sim, nSignals, gridDim, gridSize, gridOrig, D, adv=None, initLevels=None, regul=None
    ):
        # Data organisation
    ...
    def flattenIdx(self, idx):
        ...
        print(
            "flat idx = "
            + str(idx[2] + idx[1] * self.gridDim[3] + idx[0] * self.gridDim[2] * self.gridDim[3])
        )
        return idx[2] + idx[1] * self.gridDim[3] + idx[0] * self.gridDim[2] * self.gridDim[3]

    def idxFromPos(self, p):
        return (
            floor((p[0] - self.gridOrig[0]) / self.gridSize[0]),
    ...
    def trilinearWeights(self, p):
        ...
        w[0, 0, 0] = (
            (1 - dx) * (1 - dy) * (1 - dz)
            if x >= 0
            and x < self.gridDim[1]
            and y >= 0
            and y < self.gridDim[2]
            and z >= 0
            and z < self.gridDim[3]
            else 0.0
        )
        w[1, 0, 0] = (
            dx * (1 - dy) * (1 - dz)
            if x >= -1
            and x < self.gridDim[1] - 1
            and y >= 0
            and y < self.gridDim[2]
            and z >= 0
            and z < self.gridDim[3]
    ...
    def transportRates(self, signalRates, signalLevels, boundcond="constant", mode="normal"):
        ...
        for s in range(self.nSignals):
            ...
            if self.advRates:
                ...
                signalRatesView[s] += (
                    laplace(signalLevelsView[s], None, mode=boundcond, cval=boundval)
                    * self.diffRates[s]
                    / 6.0
                )
            else:
