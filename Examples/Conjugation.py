...
def setup(sim):
    ...
    sim.addCell(
        cellType=0, pos=(-5, 0, 0), dir=(1, 0, 0), length=delta, rad=0.4
    )  # acceptor
    sim.addCell(
        cellType=1, pos=(5, 0, 0), dir=(1, 0, 0), length=delta, rad=0.4
    )  # donor
    ...
    for id, cell in cells.items():
        ...
        if cell.cellType == 0:  # if a cell is an acceptor,
            for index in cell.neighbours:  # loop through all contacts
                if (
                    cells[index].cellType != 0
                ):  # if donor or transconjugant is in contact
                    # if random.random() < 0.01: # constant probability per unit time
                    if (
                        random.random() < cells[index].effGrowth / 10.0
                    ):  # probability of infection is proportional to donor growth rate
